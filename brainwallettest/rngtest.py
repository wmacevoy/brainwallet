...
class RNGTest(unittest.TestCase):
    ...
    def testInvErf(self):
        self.assertClose("ierf(1-erf(2))",self.approxInvErf(0.00467773),"2",2,abs_tol=1,rel_tol=1e-2)
        self.assertClose("ierf(1-erf(6))",self.approxInvErf(2.151e-17),"6",6,abs_tol=1,rel_tol=1e-2)

    # adapted from https://stackoverflow.com/questions/12136762/assertalmostequal-in-python-unit-test-for-collections-of-floats
    @classmethod
    def assertClose(cls,aname, a, bname, b, rel_tol=1e-9, abs_tol=0.0):
        if abs(a-b) <= max( rel_tol * max(abs(a), abs(b)), abs_tol ):
            return
        raise ValueError(str(aname) + "="+ str(a) + "is not close to " + str(bname) + "=" + str(b) + " with rel_tol=" + str(rel_tol) + " and abs_tol=" + str(abs_tol))
